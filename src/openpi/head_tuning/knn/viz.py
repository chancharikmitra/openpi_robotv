...
def _loeo_train_mask(episode_ids: np.ndarray, frame_ids: np.ndarray, query_idx: int, temp_excl_window: int):
    ...
    if len(unique_episodes) == 1:
        ...
        # Multiple episodes: exclude same episode (LOEO)
        mask = ~same_episode
    return mask


def _knn_indices(query_vec: np.ndarray, base_matrix: np.ndarray, k: int, metric: str):
    """KNN in the chosen feature subspace"""
    distances = pairwise_dist(query_vec, base_matrix, metric)
...
def knn_overlap_and_plots(
    ...
    heads_override: Optional[List[int]] = None,   # if None use model.heads
    ks=(5, 10, 20),
    feature_metric: str = None,
    action_metric: str = "mse",
    ...
    if heads_override is not None:
        ...
        feature_metric = feature_metric or (model.metric if model is not None else "cosine")
    else:
