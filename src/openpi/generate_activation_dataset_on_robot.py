    ...
    for task_name, eps in dataset.items():
        for ep_idx, ep_data in eps.items():
            ...
            for frame_idx in tqdm(key_idcs, desc=f"{task_name} ep{ep_idx:03d}", total=len(key_idcs)):
                ...
                # Guard for missing attention due to disabled flags or model differences
                # if not attention_outputs or "llm_activations" not in attention_outputs:
                #     continue

                full_attn = attention_outputs["llm_activations"]
                #print("full_attn.shape:", full_attn.shape) #shape:(1, 18, 1, 816, 8, 256)
                # Drop leading singleton dims (e.g., wrapper/scan dims) to be robust
                while full_attn.ndim > 5 and full_attn.shape[0] == 1:
                    full_attn = full_attn[0] #shape:(18, 1, 816, 8, 256)

                # FAST-style: [L, B, T, H, D]; non‑FAST: [B, T, H, D]
                if full_attn.ndim == 5:
                    last_token_attn = full_attn[:, 0, -1, :, :] #shape:(18, 8, 256)
